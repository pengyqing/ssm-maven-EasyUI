...
<%= today %>
          ...
          <td width="17%" align="left"><input id="department" name="department" type="text" style="width:200px;" /></td>
		   <td width="6%" align="right">学号：</td>
          <td width="17%" align="left"><input id="stuNumber" name="stuNumber" type="text" style="width:150px;" /></td>
          <td width="2%" style="width:50px;"><a href="javascript:void(0);searchData();">
		     查询</a></td>
          <td width="2%" style="width:90px;"><a href="javascript:void(0);clearForm();">
			清除
			</a></td>
			<td width="4%" style="width:50px;"><a href="add.jsp">
		     添加学生信息</a></td>
          <td width="4%" style="width:100px;"><a href="javascript:void(0);updateStu();">
			修改学生信息
			</a></td>
        </tr>
      </table>
     ...
     // alert('<%=basePath%>getAll.do');
       ...
       // url:'http://localhost:8080/ssm/getAll.do',
        ...
        columns: [[
					...
					{ field: 'build', title: '宿舍楼栋' ,width: 100,align:'left',sortable:true},
					{ field: 'dormNum', title: '宿舍编号' ,width: 100,align:'left',sortable:true},
					{ field: 'createTime', title: '创建时间' ,width: 150,align:'left',sortable:true},
					{ field: 'updateTime', title: '修改时间' ,width: 150,align:'left',sortable:true}
...
function deleterow(){
	...
	$.messager.confirm('提示','确定要删除吗?',function(result){
		if (result){
			...
			$.post('<%=basePath%>/del.do'+ps,function(data){
				grid.datagrid('reload'); 
...
function importFromExcel() {
	...
	/* $('#dlg_upload').dialog('open').dialog('setTitle','开发中'); */  
	<%-- $('#uploadForm').form('clear'); 
	url="<%=basePath%>/importFromExcel.do"; --%>
}
...
{
	$.messager.confirm('请确认', '您确定要导出当前的记录吗？如果数据量很大，可能需要一点时间！请确认导出条件！', function(r) {
			...
			$.messager.progress();
		  window.location.href="exportToExcel.do?department="+department+"&stuNumber="+stuNumber;
...
/* function download()
{
	 window.location.href="download.do?fileName=表格模板/表1-4院（系）教师培训进修、交流情况.xls";
...
$.each(rows,function(i,n){
    ...
    if(n.classesCard != null){
    	classesCard = n.classesCard;
    }
    if(n.build != null){
    	build = n.build;
    }
    ...
    if(i==0) {
    	window.location.href="modify.jsp?stuId="+n.id+"&stuNumber="+stuNumber+"&name="+name
		+"&sex="+sex+"&department="+department+"&grade="+grade+"&classesCard="+classesCard
		+"&build="+build+"&dormNum="+dormNum;
		}
